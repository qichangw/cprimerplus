...
float * point=rain；
/*由于rain[0]和rain[0][0]公用同一个地址rain所有可以使用指针指向rain来表示rain[0][0];但是如果我们给指针先递增然后再显示指针指向的值-显示的值为rain[1][0]对应的值，因为指针加一只是对于master array而言。*/
//初始化过程可能会出现的问题
int sq[2][3]={{5,6},{7,8}};
int sq[2][3]={5,6,7,8};
//两者不等价第一种sq[0]={5，6}；
