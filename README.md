...
10.数组的元素是变量(声明为const除外)，数组名是常量。

11.当定义为字符串时建议把字符串字面量初始化为const限定符。

12.如果是数组的话就可以使用名称改变字符串，因为数组存放的是字符串的副本。

13.使用指针时可以直接将字符串的首地址赋值给另一个指针-更高效，如果确实需要拷贝字符数组可以使用strcpy()和strncpy()。

14.字符串输入如果不在声明时初始化数组，应当明示字符串数组大小。

## 字符串函数

### gets()函数

1.gets()函数-读取整行输入直到遇到换行符然后丢弃换行符开始存储输入的字符并在字符末尾自动添加终止符'\0'.

2.gets()函数的缺陷-只知道字符串的首地址但不知道他能否装的下这些字符从而产生_缓冲区溢出_(buffer overflow)。
...
​			c：第三个参数，指明要读入的文件-因为我们都是通过键盘输入所以均为stdin(标准输入)

3.fgets()函数如果读取到换行符也会存储在字符串中不会丢弃，所以尽量不要和puts()配合使用。

4.fgets()函数应该与fputs()函数配对。

5.fgets()函数的返回值-如果读到了文件结尾则返回：空指针(null pointer)--即指向NULL
...
1.形式：gets_s()(words,STLEN)-只读取两个参数

2.gets_s()函数只从键盘中读取数据所以不需要第三个参数。

3.gets_s()函数如果读取到换行符会丢弃它而非存储他。

4.gets_s()函数如果读到了最大字符数仍未发现有换行符则将目标数组中的首字符设置为空字符丢弃随后的输入直到读取到文件结尾或换行符。-因此需要调用特殊的处理函数。

5.没有fgets()好用。这也是gets_s只是作为c库可选拓展之一
...
3.我们为什么要丢弃多余输入-因为多出来的字符会留在缓冲区，成为下一个语句的输入。
